...
# 模拟/真实搜索函数（可根据需求切换）
def search_meilisearch(query, knowledge_base, top_k, semantic_ratio):
    """调用 Meilisearch 真实搜索（支持 hybrid 语义检索）"""
    try:
...
if search_btn:
    ...
    # 调用 Meilisearch 搜索
    results, success = search_meilisearch(search_query, knowledge_base, top_k, semantic_ratio)
